...
app.get("/health", (_req, res) => {
  res.json({ status: "Notification route is Fine!" });
});
...
app.get("/metrics", async (req, res) => {
  try {
    ...
    logger.info("Notification Metrics has been scraped successfully!");
  } catch (error) {
    logger.error("Notification Metrics scraping error:", { error });
