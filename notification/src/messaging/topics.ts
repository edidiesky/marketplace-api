...
export const NotificationTopic = {
      ...
      if (email && !(await redisClient.get(notification_email_key))) {
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
          try {
            ...
            logger.info(
              "Sent NOTIFICATION_AUTHENTICATION_2FA_TOPIC data:",
              {
                token,
                email,
                fullName
  ...
  [NOTIFICATION_STORE_ONBOARDING_COMPLETED_TOPIC]: async (data: any) => {
    const {
      notificationId,
      subject,
      email,
    ...
    try {
      ...
      if (email && !(await redisClient.get(notification_email_key))) {
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
          try {
            let emailService = new EmailService();
            await emailService.sendUserStoreCreatedEmail(email, {
              subject,
              email,
              name,
              unsubscribeLink,
            ...
            logger.info(
              "Received NOTIFICATION_STORE_ONBOARDING_COMPLETED_TOPIC data:",
              {
                data,
              }
            );
            ...
            if (attempt === MAX_RETRIES) {
              logger.error(
                `[NOTIFICATION_STORE_ONBOARDING_COMPLETED_TOPIC] error`,
                {
                  message:
                    error instanceof Error
                      ? error.message
