...
docker exec development-kafka-1-1 bash -c "
  ...
  kafka-topics --create --if-not-exists --topic order.payment.refunded.topic --partitions 6 --replication-factor 3 --config min.insync.replicas=2 --bootstrap-server kafka-1:9092,kafka-2:9092,kafka-3:9092 &&
