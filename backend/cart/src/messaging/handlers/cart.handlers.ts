...
import { publishCartClearFailed } from "../publisher";
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt === MAX_RETRIES - 1) {
          publishCartClearFailed({
            orderId,
            sagaId,
            storeId,
            reason: "Cart clear failed after max retries",
            failedAt: new Date().toISOString(),
          });

          logger.error("cart_handler_clear_failed_permanently", {
            event: "cart_handler_clear_failed_permanently",
            service: SERVICE_NAME,
            orderId,
            sagaId,
            storeId,
