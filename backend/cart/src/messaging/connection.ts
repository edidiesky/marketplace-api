...
const EXCHANGE_LIST = [
  ...
  { name: EXCHANGES.CART_DLX,     type: "direct" as const },
...
const QUEUE_TOPOLOGY = [
  {
    queue:      QUEUES.ORDER_STOCK_COMMITTED,
    exchange:   EXCHANGES.ORDERS,
    routingKey: ROUTING_KEYS.ORDER_STOCK_COMMITTED,
    options: {
      durable:   true,
      arguments: {
        "x-queue-type":              "quorum",
        "x-delivery-limit":          5,
        "x-dead-letter-exchange":    EXCHANGES.CART_DLX,
        "x-dead-letter-routing-key": "cart.dead",
      },
    },
  },
  {
    queue:      QUEUES.CART_ITEM_OUT_OF_STOCK,
    exchange:   EXCHANGES.INVENTORY,
    routingKey: ROUTING_KEYS.CART_ITEM_OUT_OF_STOCK,
    options: {
      durable:   true,
      arguments: {
        "x-queue-type":              "quorum",
