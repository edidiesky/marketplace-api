...
const EXCHANGE_LIST = [
  ...
  { name: EXCHANGES.AUTHENTICATION_DLX, type: "direct" as const },
];
...
const QUEUE_TOPOLOGY = [
  {
    ...
    options: {
      ...
      arguments: {
        "x-queue-type":              "quorum",
        "x-delivery-limit":          5,
        "x-dead-letter-exchange":    EXCHANGES.AUTHENTICATION_DLX,
        "x-dead-letter-routing-key": "authentication.dead",
      },
    },
  },
  {
    queue:      QUEUES.USER_ROLLBACK,
    exchange:   EXCHANGES.AUTHENTICATION,
    routingKey: ROUTING_KEYS.ORGANIZATION_ONBOARDING_FAILED,
    options: {
      durable: true,
      arguments: {
        "x-queue-type":              "quorum",
