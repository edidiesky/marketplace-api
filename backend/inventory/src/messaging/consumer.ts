...
const queueHandlerMap: Record<string, string> = {
  ...
  [QUEUES.ORDER_PAYMENT_CONFIRMED]: ROUTING_KEYS.ORDER_PAYMENT_CONFIRMED,
};
...
export async function connectInventoryConsumer(): Promise<void> {
  ...
  for (const [queue, routingKey] of Object.entries(queueHandlerMap)) {
    const handler = inventoryHandlers[routingKey];

    if (!handler) {
      logger.warn("inventory_consumer_no_handler", {
        event:      "inventory_consumer_no_handler",
    ...
    await channel.consume(
      ...
      async (msg: ConsumeMessage | null) => {
        ...
        try {
          ...
          logger.error("inventory_consumer_parse_error", {
            event:   "inventory_consumer_parse_error",
            service: SERVICE_NAME,
            queue,
              ...
              requestContext.run(
                ...
                async () => {
                  try {
                    ...
                    logger.error("inventory_consumer_handler_error", {
                      event:      "inventory_consumer_handler_error",
                      service:    SERVICE_NAME,
                      queue,
    ...
    logger.info("inventory_consumer_started", {
      event:      "inventory_consumer_started",
      service:    SERVICE_NAME,
      queue,
      routingKey,
    });
...
export async function disconnectInventoryConsumer(): Promise<void> {
  logger.info("inventory_consumer_disconnected", {
    event:   "inventory_consumer_disconnected",
    service: SERVICE_NAME,
  });
}