...
import { productSaleRepository } from "../sales/product.sale.repository";
...
async function mvccRetry(
  ...
  for (let attempt = 0; attempt < MVCC_MAX_RETRIES; attempt++) {
    ...
    const delay = MVCC_BASE_DELAY_MS * Math.pow(2, attempt) + getMvccJitter();
...
export const inventoryService = {
  ...
  async getInventoryById(inventoryId: string): Promise<InventoryResponseDto> {
  ...
  async commitStock(dto: CommitStockDto): Promise<CommitStockResponseDto> {
    const { productId, storeId, quantity, sagaId, userId } = dto;
    const reservationKey = `inv:reservation:${sagaId}`;
    ...
    await productSaleRepository
      .record(productId, storeId, quantity)
      .catch((err) => {
        logger.warn("product_sale_log_failed", {
          event: "product_sale_log_failed",
          service: SERVICE_NAME,
          productId,
          storeId,
          sagaId,
