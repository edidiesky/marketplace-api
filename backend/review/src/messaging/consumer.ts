...
export async function connectReviewConsumer(): Promise<void> {
  const channel = getRabbitMQChannel();
  channel.prefetch(10);

  for (const [queue, routingKey] of Object.entries(queueHandlerMap)) {
    const handler = reviewHandlers[routingKey];

    if (!handler) {
      logger.warn("review_consumer_no_handler", {
        event:      "review_consumer_no_handler",
    ...
    await channel.consume(
      ...
      async (msg: ConsumeMessage | null) => {
        ...
        try {
          ...
          logger.error("review_consumer_parse_error", {
            event:   "review_consumer_parse_error",
            service: SERVICE_NAME,
            queue,
              ...
              requestContext.run(
                ...
                async () => {
                  try {
                    ...
                    logger.error("review_consumer_handler_error", {
                      event:      "review_consumer_handler_error",
                      service:    SERVICE_NAME,
                      queue,
    ...
    logger.info("review_consumer_started", {
      event:      "review_consumer_started",
      service:    SERVICE_NAME,
      queue,
      routingKey,
    });
