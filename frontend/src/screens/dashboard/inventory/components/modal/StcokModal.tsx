    ...
    <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8e6e3]">
          <div>
            <p className="text-sm  text-[#17191c] ">Update Inventory</p>
            <p className="text-xs text-[#777b86] mt-0.5">Adjust reorder point and warehouse details.</p>
          </div>
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          <div className="border border-[#e8e6e3] px-4 py-3 grid grid-cols-3 gap-4">
            {[
              ...
              <div key={s.label}>
                <p className="text-xs text-[#777b86] ">{s.label}</p>
                <p className="text-sm  text-[#17191c] ">{s.value}</p>
              </div>
            ))}
          </div>

          <label className="flex flex-col gap-1.5">
            <span className="text-xs  text-[#17191c] ">Reorder point</span>
            <input
              type="number"
              min={0}
              value={reorderPoint}
            ...
            />
            <span className="text-xs text-[#a3a6af] ">Alert triggers when available stock reaches this number.</span>
          </label>

          <label className="flex flex-col gap-1.5">
            <span className="text-xs  text-[#17191c] ">Warehouse name</span>
            <input
              type="text"
              value={warehouseName}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#e8e6e3]">
          <button onClick={onClose} className="text-sm  text-[#777b86] hover:text-[#17191c]">Cancel</button>
          <button
