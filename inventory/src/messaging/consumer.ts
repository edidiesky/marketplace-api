...
import { InventoryTopic } from "./topics";
import { sendInventoryMessage } from "./producer";
import { INVENTORY_CONSUMER_TOPICS } from "../constants";
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      await consumer.subscribe({
        topics: INVENTORY_CONSUMER_TOPICS,
        fromBeginning: false,
      });
...
async function startConsuming() {
  await consumer.run({
    ...
    eachMessage: async (payload: EachMessagePayload) => {
      ...
      let data: any;
      // getting the data value: if no data we ack the comit offset
      try {
        if (!message.value) {
      ...
      try {
        ...
        const handler = InventoryTopic[topic as keyof typeof InventoryTopic];
        if (!handler) {
...
async function sendToDLQ(
  ...
  try {
    await sendInventoryMessage("Inventory.dlq", {
