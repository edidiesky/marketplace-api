...
export const CreateRole = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      // if (parentRole) {
      //   parentRoleDoc = await Role.findById(parentRole).session(session);
      //   if (!parentRoleDoc) {
      //     res.status(BAD_REQUEST_STATUS_CODE);
      //     throw new Error("Parent role not found");
      //   }

      //   // Ensure parent role is of higher level (lower number)
      //   if (parentRoleDoc.level >= level) {
      //     res.status(BAD_REQUEST_STATUS_CODE);
      //     throw new Error(
      //       "Parent role must be of higher level than child role"
      //     );
      //   }

      //   // Ensure same directorate
      //   if (parentRoleDoc.directorate !== directorate) {
      //     res.status(BAD_REQUEST_STATUS_CODE);
      //     throw new Error("Parent role must be in the same directorate");
      //   }
      // }
          ...
          [
            {
              ...
              permissions: [
                ...
                Permission.MANAGE_TAX_STATIONS,
