...
import {
  GetAllMDAssHandler,
  GetSingleMDAssHandler,
  UpdateMDAsHandler,
  DeleteMDAsHandler,
} from "../controllers/MDAs.controller";
...
router.get("/", GetAllMDAssHandler);

// Get a single user (ADMIN/AKIRS or self)
router.get("/:id", GetSingleMDAssHandler);

// Update a user (ADMIN/AKIRS or self)
router.put("/:id",  requirePermissions([Permission.CREATE_TAX_FILING]), UpdateMDAsHandler);

// Delete a user (ADMIN/AKIRS only)
router.delete("/:id",  requirePermissions([Permission.CREATE_TAX_FILING]), DeleteMDAsHandler);
