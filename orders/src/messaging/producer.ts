...
export async function connectProducer() {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      logger.info("Order producer connected");
      return;
    } catch (error) {
...
/**
 * Send Order message with proper partitioning
 * I am paritioning by the orderId
 * Handler created here guarantees ordering for that customer's transactions
 */
export async function sendOrderMessage(topic: string, data: any, key?: string) {
  try {
    const partitionKey =
...
export async function disconnectProducer() {
  ...
  logger.info("Order producer disconnected");
}
