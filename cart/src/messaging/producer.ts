...
export async function connectProducer() {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      logger.info("Cart producer connected");
      return;
    } catch (error) {
...
/**
 * Send Cart message with proper partitioning
 * I am paritioning by the CartId
 * Handler created here guarantees Carting for that customer's transactions
 */
export async function sendCartMessage(topic: string, data: any, key?: string) {
  try {
    const partitionKey =
 ...
 */
export async function sendCartMessageBatch(
  topic: string,
...
export async function disconnectProducer() {
  ...
  logger.info("Cart producer disconnected");
}
