...
export async function connectProducer() {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      logger.info("User producer connected");
      return;
    } catch (error) {
...
/**
 * Send User message with proper partitioning
 */
 ...
 */
export async function sendUserMessageBatch(
  topic: string,
...
export async function disconnectProducer() {
  ...
  logger.info("User producer disconnected");
}
